...
summary: Heartbeat task execution prompt — Setup directive freshness gate (FIRES when last_processed missing/null OR updated > last_processed; forces directive execution; carryforward defer framing marked superseded-by-directive), Setup Declaration with structural-failure Slack alert path, Perceive Step 0 work-level judgment (Full/Skim/Minimal/Silent) with floor requirements (briefing tick override, per-source directive freshness override at full-level scope without tick-wide promotion, Immediate-tier scan, Improve), source signal check, reminder evaluation, briefing generation, Improve phase. Three Setup/Floor directives verified per directive-language.md write-time gate 2026-05-12. Ingest is a separate task (config-ingest-prompt).
updated: "2026-05-12T14:14:58Z"
cssclasses:
  - "config"
---
...
**Directive freshness gate.** For each source-config page, compute the gate state from `source-config.updated` and `source-config.last_processed`: the gate FIRES when `last_processed` is missing/null OR `updated > last_processed`; the gate does NOT fire when `last_processed` is present and `last_processed >= updated`. When the gate fires, the directive has changed (or never been applied) since the last successful sweep — this tick MUST execute the source-config's current `## Directives` section (as read in this Setup pass) on its in-window set, where "in-window set" is defined by the current (this-pass) directive's own scope rules. MUST NOT defer. MUST NOT surface a Decision item requesting approval to execute behavior the current directive already mandates as MUST. Carryforward briefing context from prior ticks does not override the directive when this gate fires — when a carryforward Decision item's recommended action overlaps with the now-mandated behavior, MUST mark the carryforward item as `superseded-by-directive` in the tick output and MUST execute the directive instead. Gate evaluation occurs at Setup before Step 0's work-level judgment and is not subject to per-tick judgment override.

**Declaration.** For each source-config where the gate fires, MUST emit `Directive freshness gate: source=<title>, updated=<ISO>, last_processed=<ISO>|null, status=execute` in the tick output during Setup (use literal `null` when `last_processed` is missing). MUST NOT emit any freshness-gate line for sources where the gate does not fire. Detection of an absent declaration on a source whose gate fired is a structural failure: MUST emit `Structural failure: directive freshness declaration missing for <title>` as the first line of the tick output AND MUST dispatch an Immediate-tier alert via `slack_send_message_draft` to the user DM (user ID from config-user) with that same text as the message body. The tick MUST continue to Step 1 after these emissions — structural failure surfaces the gap without aborting otherwise-recoverable work.
...
- **Directive freshness override.** If the Setup directive freshness gate fired for one or more source-configs (regardless of selected work level), MUST execute the full pipeline (Step 1 + Predict + Plan + Act) for each gate-fired source on this tick, matching the scope a `full`-level tick would apply to that source. The override is per-source — sources without a gate-fire follow the selected level's normal scope. MUST NOT promote the tick's overall level to `full`; only the gate-fired source(s) receive `full`-level processing. Briefing emission for gate-fired sources follows the existing briefing-tick rule (items accumulate to the next briefing tick; emitted only when this tick is also the briefing tick).
