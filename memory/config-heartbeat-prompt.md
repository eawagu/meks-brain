...
summary: Heartbeat task execution prompt — Setup directive freshness gate (updated > last_processed forces directive execution, overrides carryforward defer framing), Perceive Step 0 work-level judgment (Full/Skim/Minimal/Silent) with floor requirements (briefing tick override, directive freshness override, Immediate-tier scan, Improve), source signal check, reminder evaluation, briefing generation, Improve phase. Ingest is a separate task (config-ingest-prompt).
updated: "2026-05-12T13:50:04Z"
cssclasses:
  - "config"
---
...
**Directive freshness gate.** For each source-config page, compare `source-config.updated` to `source-config.last_processed`. When `updated > last_processed`, the directive has changed since the last successful sweep — this tick MUST execute the current directive on the source's in-window set during Step 1. MUST NOT defer. MUST NOT surface a Decision item requesting approval to execute behavior the directive already mandates as MUST. Carryforward briefing context from prior ticks does not override the directive when this gate fires — when a carryforward Decision item's recommended action overlaps with the now-mandated behavior, treat the carryforward item as superseded by the directive update (note it as `superseded-by-directive` in the tick output) and execute the directive instead. The comparison is deterministic; gate evaluation occurs at Setup before Step 0's work-level judgment and is not subject to per-tick judgment override.

**Declaration:** For each source-config where the gate fires, MUST emit `Directive freshness gate: source=<title>, updated=<ISO>, last_processed=<ISO>, status=execute` in the tick output during Setup. Absence of the declaration on any source whose gate fired indicates Setup did not run the gate — treat as structural failure.
...
- **Directive freshness override.** If the Setup directive freshness gate fired for any source-config (regardless of selected level), MUST execute that source's Step 1 processing on this tick. The gate is a per-source override that elevates the affected source(s) into Step 1 even on `skim`/`minimal`/`silent` ticks. Other sources without a gate-fire follow the selected level's normal scope.
