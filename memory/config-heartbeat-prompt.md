...
- **Source-config write scope.** When the heartbeat updates a source-config page via `update_page`, only the `last_processed` frontmatter field may change. The `body` parameter MUST equal the page's body content as it existed before the current tick began. MUST NOT add, remove, or modify any body content (including `## Notes`, `## Connection`, `## Directives`, or any other section) during heartbeat tick processing. Tick-level audit lives in git history (every `update_page` produces a commit); operational state requiring cross-tick continuity lives in frontmatter.
