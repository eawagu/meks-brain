...
summary: "Five CRITICAL vulnerabilities. TDSD-6479 Harness migration CTO-approved Apr 12. DCIR failure 100% overnight Apr 14 (03:50–04:06 WAT). Apr 15 19:06 WAT: DCIR alert resumed at 40.65% (Wema route) after ~39h silence, ~7h23min post-remediation — Wema DB script did NOT close failure-rate generator."
updated: "2026-04-15T18:14:35Z"
cssclasses:
  - "situation"
...
**Apr 15 19:09 WAT — DCIR ALERT RESUMED (NEW):** After ~39h of silence (04:06 WAT Apr 14 → 19:06 WAT Apr 15), DCIR Monitoring Service fired again — Transaction High Failure Warning, rate **40.65%**, threshold 20%. Reply-To `wemaalert@wemabank.com` (Wema DCIR route, same as Apr 14 chain). Arrives ~7h23min after [[Wema Bank]] DB remediation script executed 11:43 WAT. Implication: Wema-side settlement-update script did NOT close the DCIR failure-rate generator — the failure and the stuck-settlement remediation are addressing different layers. Single alert so far (not 100%, not escalating yet). Classified briefing-tier, not Immediate — no P1 filed in Slack/email, not yet a cycle. Silence rule for active P1 N/A (no P1 declared). Pattern-significant: same route breaching threshold 24h apart post-remediation. Covered under [[Wema Bank — RC91 After Settlement Resolution]] delta — structural retro ask (briefing-2026-04-15 B3) remains open.

**Apr 15 11:09 WAT — Parallex/Ecobank/FCMB DCIR-chain activity:**

1. **Parallex Bank DCIR DB INDEX JOB FAILURE (11:09 WAT):** Segun Ogunsola (Parallex Bank, Channels Switching) → [[Babajide Ojoboorun]] + aptpaytechnicalsupport: "We observed the Index job as regards the DCIR database is failing as seen in the screenshot below. Kindly assist review with the team and advise." Distinct from previous Parallex DCIR signals (Authentication Error Message Exposure). Database index failure can degrade DCIR portal query performance and downstream reconciliation. Briefing-tier — new Parallex DCIR issue pending Babajide triage.
...
- 2026-04-15 11:09 WAT — 29h+ silence on DCIR monitoring holds. Four new DCIR-chain signals this tick: Parallex DCIR DB INDEX JOB FAILURE; Parallex Authentication Error remediation in flight; Ecobank DCIR CLAIMS IN PROGRESS; FCMB credentials chaser.
- 2026-04-15 19:09 WAT — **DCIR monitoring alert resumed after ~39h silence.** 19:06 WAT: 40.65% failure warning (threshold 20%), Reply-To wemaalert@wemabank.com (Wema route). ~7h23min post Wema DB remediation script execution. Remediation did NOT close DCIR failure-rate generator — settlement-update script is a different layer from the DCIR failure root cause. Single alert, not escalating, no P1 filed. Briefing-tier for Apr 16 briefing. Pattern signal for [[Wema Bank — RC91 After Settlement Resolution]].